...
AuctionData:RegisterDB("AuctionDataDB");
...
function AuctionData:OnEnable()
	...
	self:WatchDefaultItems();
end;

function AuctionData:WatchDefaultItems()
...
function AuctionData:GetItemDataCount(aItemName)
	local info = self:GetItemTable(aItemName);

	if(info ~= nil) then
		return info.myDataCount;
	end;
...
function AuctionData:UpdateFailedSale(aItem, aMoney)
	...
	self.db.profile.mySalesRecord[aItem].myFailedSales = self.db.profile.mySalesRecord[aItem].myFailedSales + 1;
	return;
end;
